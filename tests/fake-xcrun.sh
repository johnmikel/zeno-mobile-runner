...
JSON
  ...
  uninstall)
    ...
    ;;
  openurl)
    [[ "${2:-}" == "fake-ios-1" && ( "${3:-}" == "exampleapp:///e2e-auth?probe=1" || "${3:-}" == "https://example.com/e2e-auth?probe=1" || "${3:-}" == "exp+exampleapp://expo-development-client/?url=http%3A%2F%2F127.0.0.1%3A8081" ) ]] || {
