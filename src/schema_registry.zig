...
const public_schemas = [_]PublicSchema{
    ...
    .{ .name = "action-registry", .path = "schemas/action-registry.schema.json", .id = "https://zmr.dev/schemas/action-registry.schema.json", .description = "Canonical cross-protocol action metadata and migration contract" },
    ...
    .{ .name = "test-report", .path = "schemas/test-report.schema.json", .id = "https://zmr.dev/schemas/test-report.schema.json", .description = "Machine-readable zmr test workspace aggregate report with per-attempt artifacts" },
