...
fn diagnoseFailure(allocator: std.mem.Allocator, content: []const u8, err: anyerror) !Diagnostic {
    ...
    return switch (err) {
        ...
        => try pathDiagnostic(allocator, content, "$.steps[].key", "key"),
        error.StepMissingX1,
