...
test "draft from trace writes conservative surface smoke scenario" {
    ...
    try std.fs.cwd().writeFile(.{
        ...
        .data =
        \\{"schemaVersion":1,"runnerVersion":"0.1.8","protocolVersion":"2026-04-28","scenarioName":"login smoke","appId":"com.example.mobiletest","status":"passed","startedAtMs":1,"endedAtMs":2,"durationMs":1,"failedStepIndex":null,"error":null,"eventsPath":"events.jsonl","artifactsDir":"artifacts","eventCount":3,"snapshotCount":2,"partialFailureCount":0,"reportPath":null}
        \\
        ,
    });
    try std.fs.cwd().writeFile(.{
...
test "draft from trace can replay successful supported actions" {
    ...
    try std.fs.cwd().writeFile(.{
        .sub_path = trace_dir ++ "/trace.json",
        .data =
        \\{"schemaVersion":1,"runnerVersion":"0.1.8","protocolVersion":"2026-04-28","scenarioName":"agent session","appId":"com.example.mobiletest","status":"passed","startedAtMs":1,"endedAtMs":2,"durationMs":1,"failedStepIndex":null,"error":null,"eventsPath":"events.jsonl","artifactsDir":"artifacts","eventCount":14,"snapshotCount":1,"partialFailureCount":0,"reportPath":null}
        \\
        ,
    });
    try std.fs.cwd().writeFile(.{
        ...
        \\{"seq":10,"timestampMs":10,"kind":"assert.visible","payload":{"status":"ok","selector":{"text":"Dashboard"},"timeoutMs":2500}}
        \\{"seq":11,"timestampMs":11,"kind":"assert.notVisible","payload":{"status":"ok","selector":{"textContains":"Spinner"},"timeoutMs":2600}}
        \\{"seq":12,"timestampMs":12,"kind":"assert.noneVisible","payload":{"status":"ok","selectors":[{"textContains":"Crash"},{"textContains":"Fatal"}],"timeoutMs":2700}}
        \\{"seq":13,"timestampMs":13,"kind":"assert.healthy","payload":{"status":"ok","timeoutMs":3000}}
        \\{"seq":14,"timestampMs":14,"kind":"scenario.end","payload":{"value":"agent session","status":"passed"}}
        \\
        ,
    });
    try std.fs.cwd().writeFile(.{
    ...
    try std.testing.expectEqual(@as(usize, 13), result.summary.step_count);
    try std.testing.expect(result.summary.replay.enabled);
    try std.testing.expectEqual(@as(usize, 12), result.summary.replay.event_count);
    try std.testing.expectEqual(@as(usize, 11), result.summary.replay.step_count);
    ...
    try std.testing.expect(std.mem.indexOf(u8, scenario, "\"action\":\"assertVisible\",\"selector\":{\"text\":\"Dashboard\"},\"timeoutMs\":2500") != null);
    try std.testing.expect(std.mem.indexOf(u8, scenario, "\"action\":\"assertNotVisible\",\"selector\":{\"textContains\":\"Spinner\"},\"timeoutMs\":2600") != null);
    try std.testing.expect(std.mem.indexOf(u8, scenario, "\"action\":\"assertNoneVisible\",\"selectors\":[{\"textContains\":\"Crash\"},{\"textContains\":\"Fatal\"}],\"timeoutMs\":2700") != null);
    try std.testing.expect(std.mem.indexOf(u8, scenario, "\"action\":\"assertHealthy\",\"timeoutMs\":3000") != null);
