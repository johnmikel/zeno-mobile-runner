...
test "pilot scenario examples parse" {
    ...
    // health: it waits for the launcher's persistent marker to disappear
    // ("evelopment servers" covers both case-sensitive spellings) instead of
    // matching transient loading text or launcher-ambiguous tokens like
    // "Home", which previously produced false greens and false timeouts.
    try std.testing.expectEqual(@as(std.meta.Tag(scenario.Step), .wait_not_visible), std.meta.activeTag(ios_dev_client.steps[2]));
    try std.testing.expectEqualStrings("evelopment servers", ios_dev_client.steps[2].wait_not_visible.selector.text_contains.?);
    try std.testing.expectEqual(@as(std.meta.Tag(scenario.Step), .assert_none_visible), std.meta.activeTag(ios_dev_client.steps[3]));
    try std.testing.expectEqualStrings("Unable to load", ios_dev_client.steps[3].assert_none_visible.selectors[0].text_contains.?);
    try std.testing.expectEqualStrings("There was a problem loading", ios_dev_client.steps[3].assert_none_visible.selectors[1].text_contains.?);
    ...
    try std.testing.expectEqual(@as(std.meta.Tag(scenario.Step), .wait_not_visible), std.meta.activeTag(ios_dev_client_route.steps[2]));
    try std.testing.expectEqual(@as(std.meta.Tag(scenario.Step), .assert_none_visible), std.meta.activeTag(ios_dev_client_route.steps[4]));
