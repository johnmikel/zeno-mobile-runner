...
test "app scaffold writes config smoke scenarios and gitignore without overwriting" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, config, "\"androidReport\": \"zmr report traces/zmr-android --out traces/zmr-android/report.html --junit traces/zmr-android/junit.xml\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, config, "\"androidReliability\": \"export ZMR_BIN=\\\"${ZMR_BIN:-zmr}\\\"; zmr-benchmark --zmr .zmr/android-smoke.json --device emulator-5554 --app-id com.example.mobiletest --runs 20 --trace-root traces/zmr-android-reliability --min-pass-rate 100 --max-failures 0 --max-p95-ms 30000 && \\\"$ZMR_BIN\\\" report traces/zmr-android-reliability --out traces/zmr-android-reliability/report.html --junit traces/zmr-android-reliability/junit.xml\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, config, "\"iosReport\": \"zmr report traces/zmr-ios --out traces/zmr-ios/report.html --junit traces/zmr-ios/junit.xml\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, config, "\"iosReliability\": \"export ZMR_BIN=\\\"${ZMR_BIN:-zmr}\\\"; zmr-benchmark --zmr .zmr/ios-smoke.json --platform ios --device booted --app-id com.example.mobiletest --xcrun xcrun --runs 20 --trace-root traces/zmr-ios-reliability --min-pass-rate 100 --max-failures 0 --max-p95-ms 45000 && \\\"$ZMR_BIN\\\" report traces/zmr-ios-reliability --out traces/zmr-ios-reliability/report.html --junit traces/zmr-ios-reliability/junit.xml\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, agent, "traces/zmr-android-reliability/junit.xml") != null);
