...
compat_alias="mae""stro"
"$ZMR" import "$compat_alias" "$TMPDIR/flow.yaml" --out "$TMPDIR/.zmr/compat-imported.json" --json > "$TMPDIR/import-compat.json"
grep -q '"ok":true' "$TMPDIR/import-compat.json"
grep -q '"format":"flow-yaml"' "$TMPDIR/import-compat.json"
grep -q '"out":"'"$TMPDIR"'/.zmr/compat-imported.json"' "$TMPDIR/import-compat.json"
grep -q '"stepCount":10' "$TMPDIR/import-compat.json"
"$ZMR" validate "$TMPDIR/.zmr/compat-imported.json"
